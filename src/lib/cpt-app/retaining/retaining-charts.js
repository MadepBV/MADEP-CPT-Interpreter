 ...
 * Crisp on HiDPI, theme colours from the app palette, no external libraries.
 */

const FONT = '11px JetBrains Mono, monospace';
const AXIS = 'rgba(24,24,26,0.55)';
const GRID = 'rgba(24,24,26,0.08)';
const TEXT = '#4a4a52';
...
export function drawDepthChart(canvas, spec) {
  ...
  if (vMin < 0 && vMax > 0) { ctx.strokeStyle = 'rgba(24,24,26,0.35)'; ctx.setLineDash([3, 3]); ctx.beginPath(); ctx.moveTo(X(0), mT); ctx.lineTo(X(0), mT + ph); ctx.stroke(); ctx.setLineDash([]); }
  // ticks
  ...
  if (spec.title) { ctx.textAlign = 'left'; ctx.textBaseline = 'top'; ctx.fillStyle = '#18181a'; ctx.font = '600 11.5px DM Sans, system-ui, sans-serif'; ctx.fillText(spec.title, mL, 4); ctx.font = FONT; }
  // markers (excavation, anchor, toe)
  for (const mk of spec.markers || []) {
    if (!Number.isFinite(mk.depth)) continue;
    ctx.strokeStyle = mk.color || 'rgba(24,24,26,0.45)'; ctx.setLineDash([4, 3]); ctx.beginPath(); ctx.moveTo(mL, Y(mk.depth)); ctx.lineTo(mL + pw, Y(mk.depth)); ctx.stroke(); ctx.setLineDash([]);
    ctx.fillStyle = mk.color || TEXT; ctx.textAlign = 'right'; ctx.textBaseline = 'bottom'; ctx.fillText(mk.label, mL + pw - 3, Y(mk.depth) - 2);
  }
  // series
  ...
  for (const s of spec.series) {
    ...
    ctx.fillStyle = TEXT; ctx.fillText(s.label, lx + 14, ly);
    ly += 14;
  }
}
...
export function drawXYChart(canvas, spec) {
  ...
  if (spec.title) { ctx.textAlign = 'left'; ctx.textBaseline = 'top'; ctx.fillStyle = '#18181a'; ctx.font = '600 11.5px DM Sans, system-ui, sans-serif'; ctx.fillText(spec.title, mL, 4); ctx.font = FONT; }
  for (const h of spec.hlines || []) { ctx.strokeStyle = h.color || '#9b3a32'; ctx.setLineDash([5, 3]); ctx.beginPath(); ctx.moveTo(mL, Y(h.y)); ctx.lineTo(mL + pw, Y(h.y)); ctx.stroke(); ctx.setLineDash([]); ctx.fillStyle = h.color || '#9b3a32'; ctx.textAlign = 'right'; ctx.textBaseline = 'bottom'; ctx.fillText(h.label, mL + pw - 3, Y(h.y) - 2); }
  ...
  for (const v of (spec.vlines || []).slice().sort((a, b) => a.x - b.x)) {
    ctx.strokeStyle = v.color || 'rgba(24,24,26,0.45)'; ctx.setLineDash([4, 3]); ctx.beginPath(); ctx.moveTo(X(v.x), mT); ctx.lineTo(X(v.x), mT + ph); ctx.stroke(); ctx.setLineDash([]);
    const x0 = X(v.x) + 3, w = ctx.measureText(v.label).width;
    row = x0 < lastLabelEnd + 6 ? row + 1 : 0;
    lastLabelEnd = Math.max(lastLabelEnd, x0 + w);
    ctx.fillStyle = v.color || TEXT; ctx.textAlign = 'left'; ctx.textBaseline = 'top'; ctx.fillText(v.label, x0, mT + 2 + row * 12);
  }
  for (const s of spec.series) {
  ...
  let lx = mL + 8, ly = mT + 8;
  ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
  for (const s of spec.series) { ctx.fillStyle = s.color; ctx.fillRect(lx, ly - 4, 10, 8); ctx.fillStyle = TEXT; ctx.fillText(s.label, lx + 14, ly); ly += 14; }
}
