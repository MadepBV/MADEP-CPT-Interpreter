...
function soilInputs(key, m) {
  return `<div class="st6-rw-soilgrid">
    ...
    <label class="st6-rw-field"><span>Framework</span><select onchange="retwallSetBool('${key}.drained', this.value === 'drained')"><option value="drained"${m.drained !== false ? ' selected' : ''}>drained (φ′, c′)</option><option value="undrained"${m.drained === false ? ' selected' : ''}>undrained (c<sub>u</sub>)</option></select></label>
  </div>`;
}
...
function overrideCell(key, field, base, ov, step, digits) {
  ...
  return `<td><input class="${has ? 'ov' : ''}" type="number" step="${step}" placeholder="${fmt(base, digits)}" value="${esc(val)}" onchange="retwallOverride('${esc(key)}','${field}',this.value)"><span class="base">${fmt(base, digits)}</span></td>`;
}
...
export function soilPanel(rw, layers) {
  ...
  let body = `<label class="st6-rw-field"><span>Soil source</span>
  ...
  layers.forEach((L, i) => {
    ...
    rows += `<tr>
      <td title="${esc(L.type || '')}${L.subtype ? ' — ' + esc(L.subtype) : ''}"><strong>${esc(L.type || 'layer')}</strong><span class="base">${fmt(L.top, 2)}–${fmt(L.bot, 2)} m · q<sub>c</sub> ${fmt(L.avgQc, 1)}</span></td>
      ...
      <td><select style="font-size:10px" onchange="retwallOverrideDrained('${esc(key)}', this.value)"><option value="base"${o.drained === undefined || o.drained === null ? ' selected' : ''}>auto (${base.drained ? 'dr' : 'undr'})</option><option value="drained"${o.drained === true ? ' selected' : ''}>drained</option><option value="undrained"${o.drained === false ? ' selected' : ''}>undrained</option></select></td>
    </tr>`;
    void drained;
  });
  body += `<div class="st6-rw-card-title" style="margin-top:10px">Characteristic parameters — override where needed</div>
    <div class="st6-rw-layerwrap"><table class="st6-rw-layers">
      <thead><tr><th>Layer</th><th>γ</th><th>γ<sub>sat</sub></th><th>φ′</th><th>c′</th><th>c<sub>u</sub></th><th>framework</th></tr></thead>
      <tbody>${rows}</tbody></table></div>
    <div class="st6-rw-actions">
      <span class="st6-rw-field" style="gap:4px"><span>c′ for all layers</span><input type="number" step="0.5" min="0" style="width:56px" id="retwallAllC" value="0.5"><button type="button" class="st6-rw-copy" onclick="retwallSetAllC(document.getElementById('retwallAllC').value)">apply</button></span>
      <button type="button" class="st6-rw-copy" onclick="retwallClearOverrides()">clear overrides</button>
    </div>
