...
function datasheetForm(vb) {
  ...
  let html = `${textRow('Sheet / model', p('name'), s.name, { placeholder: 'e.g. HST030 RM-DFS-CW30, ICE 28RF' })}
    <div class="st6-rw-soilgrid">${numberRow('Centrifugal force', p('force_kN'), s.force_kN ?? '', { unit: 'kN', step: 5, min: 0, title: 'The force printed on the sheet (at the maximum rpm unless you set "stated at" below)' })}${numberRow('Frequency max', p('rpmMax'), s.rpmMax ?? '', { unit: 'rpm', step: 50, min: 0 })}</div>
    <label class="st6-rw-field"><span>Attached mass from</span>${segmented(p('massSource'), massSrc, [{ value: 'amplitude', label: 'Amplitude' }, { value: 'mass', label: 'Vibrating mass' }])}</label>
    ${massSrc === 'amplitude'
  ...
  const carrier = `<div class="st6-rw-card-title">From the sheet</div>
    <div class="st6-rw-soilgrid">${numberRow('Oil flow', p('flow_lmin'), s.flow_lmin ?? '', { unit: 'l/min', step: 5, min: 0 })}${numberRow('…max', p('flowMax_lmin'), s.flowMax_lmin ?? '', { unit: 'l/min', step: 5, min: 0 })}</div>
    ...
    <div class="st6-rw-card-title" style="margin-top:8px">Your carrier</div>
...
export function drivabilityPanel(rw) {
  ...
  const overdig = Number.isFinite(rw.result?.overdigUls) ? rw.result.overdigUls : 0.30;
  const target = targetToeDepth(rw);
  let body = `<label class="st6-rw-field"><span>Installation</span>${segmented('drivability.method', dr.method, [{ value: 'vibratory', label: 'Vibratory' }, { value: 'impact', label: 'Impact' }, { value: 'push', label: 'Push (static)' }])}</label>
  ...
  const modelParams = `${numberRow('Target toe depth override', 'drivability.targetDepth', dr.targetDepth ?? '', { unit: 'm', step: 0.1, title: 'Blank = H + over-dig + embedment (pile driven from the retained surface)' })}
    <div class="st6-rw-card-title" style="margin-top:8px">Resistance profile from the CPT</div>
  ...
  if (dr.method === 'vibratory') {
    ...
    body += `<div class="st6-rw-card-title" style="margin-top:10px">Static push</div>
  ...
  const impactAdvanced = dr.method === 'impact' ? `<div class="st6-rw-card-title" style="margin-top:8px">Impact hammer details</div>
      <div class="st6-rw-soilgrid">${numberRow('Helmet mass', 'drivability.hammer.helmetMass', dr.hammer.helmetMass, { unit: 'kg', step: 100 })}${numberRow('Cushion stiffness', 'drivability.hammer.cushionStiffness', dr.hammer.cushionStiffness, { unit: 'kN/m', step: 1e5 })}</div>
      ${numberRow('Cushion COR', 'drivability.hammer.cushionCor', dr.hammer.cushionCor, { step: 0.05, min: 0.3, max: 1 })}
      <div class="st6-rw-card-title" style="margin-top:8px">Smith soil model</div>
...
export function drivabilityView(rw) {
  ...
  if (R.push) {
    ...
    html += `<div class="st6-rw-grid2"><div>
      ...
      <div class="st6-rw-grid2" style="margin-top:12px"><div></div><div>
      ...
      <div class="st6-rw-actions">${copyButton('copy per-depth table (TSV)', toTsv(['z_m', 'Rs_kN', 'Rb_kN', 'Rstatic_kN', 'W_kN', 'Frequired_kN', 'G_kN'], P.perDepth.map((r) => [r.z, r.Rs_kN, r.Rb_kN, r.Rstatic_kN, r.W_kN, r.Frequired_kN, r.G_kN])))}</div>
      </div></div>`;
  }
  if (R.impact) {
    const I = R.impact;
...
export function drawDrivabilityCharts(rw) {
  ...
  if (R.push && c1) {
    ...
    const markers = [{ depth: R.target, label: `target ${fmt(R.target, 2)} m`, color: '#4a4a52' }];
    if (P.refusalDepth_m != null) markers.push({ depth: P.refusalDepth_m, label: `refusal ${fmt(P.refusalDepth_m, 2)} m`, color: '#b43c32' });
    else if (P.refusalDepth125_m != null) markers.push({ depth: P.refusalDepth125_m, label: `refusal (m_R 1.25) ${fmt(P.refusalDepth125_m, 2)} m`, color: '#8a620d' });
    drawDepthChart(c1, { title: 'Static resistance vs push force — where the balance closes', unit: 'kN', depthMax: Math.max(...z), series: [{ z, v: P.perDepth.map((r) => r.Rstatic_kN), color: '#2e6f55', label: 'R_static (upper envelope)' }, { z, v: P.perDepth.map((r) => r.Frequired_kN), color: '#9b3a32', label: 'F required (R − W)' }, { z, v: P.perDepth.map(() => P.force_kN), color: '#7e50a8', label: `F_push ${fmt(P.force_kN, 0)} kN`, width: 1.4 }], markers });
  }
  if (R.impact) {
    const I = R.impact;
