...
export function structuralView(rw, result, structural) {
  ...
  if (!structural?.section) return '<div class="st6-rw-note warn">Select a catalogue section to run the steel checks.</div>';
  ...
  const cols = [
    { label: 'Verification', render: (r) => `<strong>${esc(r.label)}</strong><div class="st6-rw-checksub">${esc(r.ref || '')}${r.note ? ' · ' + esc(r.note) : ''}</div>` },
    ...
    { label: '', render: (r) => r.info ? '<span class="st6-rw-badge info">info</span>' : badge(r.pass) }
  ];
  let html = `<div class="st6-rw-grid2"><div><div class="st6-rw-card-title">Section resistance</div>${resistance}</div>
    <div><div class="st6-rw-card-title">Design effects (STR envelope)</div>${kvList([['M<sub>Ed</sub>', fmt(result.structural.Mmax, 2), `${uM} · ${result.structural.combo}`], ['V<sub>Ed</sub>', fmt(result.structural.Vmax, 2), `${uF} · ${result.structural.vCombo}`], ...(result.structural.anchorForce > 0 ? [['T<sub>Ed</sub>', fmt(result.structural.anchorForce, 2), `${uF} · ${result.structural.anchorCombo}`]] : [])])}</div></div>
    <div class="st6-rw-card-title" style="margin-top:10px">Steel checks</div>${table(cols, S.steel.rows)}`;
  if (S.lagging) {
    const L = S.lagging;
    html += `<div class="st6-rw-card-title" style="margin-top:10px">Lagging plate — ${fmt(rw.soldier.laggingThk * 1000, 0)} mm ${esc(rw.soldier.laggingGrade)}, span ${L.spanMode === 'clear' ? 'clear s − b' : 'centre-to-centre'} = ${fmt(L.L, 3)} m</div>
      ...
      <div class="st6-rw-note">Elastic verification on the c/c span is the conservative basis (Rekennota §7.8); the clear span between the flanges is the physically correct one. No arching between the piles is credited.</div>`;
  }
  if (S.vertical) {
    const V = S.vertical;
    html += `<div class="st6-rw-card-title" style="margin-top:10px">Vertical equilibrium of one pile</div>
      ...
      <div class="st6-rw-note">Base resistance F<sub>max</sub> not credited. With δ = 0 on the passive face no vertical earth-pressure component acts on the pile.</div>`;
  }
  return html;
}
...
function gravityStructural(result) {
  ...
  return `<div class="st6-rw-card-title">Structural design forces (per m run)</div>
    <table class="st6-rw-table"><tbody>
    ...
    <div class="st6-rw-note">Reinforcement design to EN 1992 remains the engineer's task (see the reinforcement application for a strip screening).</div>`;
}
