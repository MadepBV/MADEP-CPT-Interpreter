...
export function branchesView(rw, result) {
  if (!result?.branches) return '<div class="card card--quiet card--note">Branches are reported for embedded walls only.</div>';
  ...
  const cards = result.branches.map((b) => {
    ...
    const f = b.factors || {};
    return `<div class="card card--branch${gov ? ' is-selected' : ''}" data-branch="${esc(b.id)}">
      <h4 class="card__title">${esc(b.id)}${gov ? ' <span class="pill pill--good pill--data">governs</span>' : ''}</h4>
      <div class="mono ink-muted">γ<sub>G</sub> ${fmt(f.gG, 2)}${f.gGResist !== f.gG ? ` / ${fmt(f.gGResist, 2)} pass.` : ''} · γ<sub>Q</sub> ${fmt(f.gQ, 2)} · γ<sub>φ</sub> ${fmt(f.gPhi, 2)} · γ<sub>c</sub> ${fmt(f.gC, 2)}${f.effectFactor !== 1 ? ` · effects × ${fmt(f.effectFactor, 2)}` : ''}</div>
      <div class="kv">
      ...
      ${!b.bracketed ? '<div class="verdict verdict--warn verdict--inline">No equilibrium within 40 m of embedment.</div>' : ''}${!b.closed ? '<div class="verdict verdict--warn verdict--inline">Moment diagram not closed on the provided pile (under-embedded for this branch).</div>' : ''}
    </div>`;
  }).join('');
  ...
  return `<div class="card-grid">${cards}</div>
    <div class="actions">${seg}<span class="field__hint">design strengths and coefficients of the selected branch</span>${copyButton('copy branch table (TSV)', tsv)}</div>
    <div class="card__eyebrow">Retained side</div>${table(layerCols, sel.back || [])}
    <div class="card__eyebrow">Excavation side</div>${table(layerCols, sel.front || [])}
    <div class="card__text">${esc(sel.label)}. Design strengths: tan φ′<sub>d</sub> = tan φ′<sub>k</sub>/γ<sub>φ</sub>, c′<sub>d</sub> = c′<sub>k</sub>/γ<sub>c</sub>, c<sub>u,d</sub> = c<sub>u,k</sub>/γ<sub>cu</sub>; coefficients recomputed from φ′<sub>d</sub> (never K<sub>p</sub>/γ). ${result.perPile ? 'All forces per pile (× spacing above the excavation; b / b<sub>eff</sub> below).' : 'All forces per metre of wall.'}</div>`;
}
