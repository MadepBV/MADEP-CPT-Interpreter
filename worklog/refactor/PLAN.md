...
- The tool rail's View *card* is unreachable (`activePanel` ternary) — its body and two `data-st6details` groups are dead HTML (PR 18f).
- Three write-only UI flags: `ui.bishopSettingsCollapsed` (the render hard-codes `settingsCollapsed = true`, so Hide only re-renders) and `ui.bishopToolRailExpanded` (no reader).
- Five blocks are duplicated between the settings column and the canvas sheets (drains, walls, BCs, regions, materials) — folding them is a behaviour change for PR 19.
- A `seepslope/results/` package would drop 8 env hooks from the panels and 6 from the canvas (PR 18f finding 5).
